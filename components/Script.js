...
async function filterByGroupName(GroupName) {
    ...
    var DataList = JsonData.filter(item => item.group === GroupName);           // JsonData'yı filter yaparak GroupName adı verdiğim parametreye eşitledim.
    getGroups(DataList);                                                        // getGroups fonksiyonunu çağırarak DataList'i parametre olarak yolladım.
};
...
filterByGroupName("SteelBlue");                                                 // Bu fonksiyon ile filtrelemek istediğimiz grup ismini giriyoruz
